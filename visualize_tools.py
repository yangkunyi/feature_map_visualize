...
def get_robust_pca(features: Tensor, m: float = 2, remove_first_component=False):
    ...
    if remove_first_component:
        ...
        reduction_mat = torch.pca_lowrank(features[fg_mask], q=3, niter=1)[2]
        colors = features @ reduction_mat
    else:
...
def get_pca_map(
    ...
    if img_size == None:
        img_size = feature_map.shape[1:3]
...
def get_similarity_map(features: Tensor, img_size=None):
    """
    compute the similarity map of the central patch to the rest of the image
    """
    assert len(features.shape) == 4, "features should be (1, H, W, C)"
    if img_size == None:
        img_size = features.shape[1:3]
